...
album_d <- as.data.frame(fromJSON(file="analysis/corrected_data/album_corrected.json"))
game_d <- as.data.frame(fromJSON(file="analysis/corrected_data/game_corrected.json"))
## Create wd matrices
...
bstrap_similarity <- function(m, measure, n_sims=100) {
  ...
  for (i in 2:ncol(m)) {
      for (j in 1:(i - 1)) {
        # v1 <- m[,i]
        # v2 <- m[,j]
        sims <- bstrap_sims(measure,  m[,i], m[,j], n_sims)
...
m <- matrix(c(v1, v2), nrow=3)
apply(m, MARGIN=2, smooth_vec)
...
jsd <- function(v1, v2) {
  if (0 %in% v1 || 0 %in% v2) stop("zeros in one or both vectors, please smooth")
...
smoothed_m <- apply(apply(as.matrix(test_jsd[, -1]), MARGIN=2, smooth_vec), MARGIN=2, norm_vec)
bstrap_similarity(smoothed_m, jsd, n_sims=10)
smoothed_m %>% View

0 %in% smoothed_m

jsd(norm_vec(smooth_vec(aggregate_df2$bad_terrible_book)),
    norm_vec(smooth_vec(aggregate_df2$bad_terrible_movie)))
```
