...
d_joined_model <- rbind(two_alt_m %>% mutate(model="2"),
                        three_alt_m %>% mutate(model="3"),
                        four_alt_m %>% mutate(model="4"),
                        five_alt_m %>% mutate(model="5"))
# write.csv(d_joined_model, "/Users/benpeloquin/Desktop/Projects/scalar_alts/analysis/full_data.csv")
```
...
What happens when we simply add alternatives and assume `depth=1`, without tuning `alpha`.
```{r exp1_reproduce}
d_joined_models <- rbind(two_alt_m %>% mutate(model="2"),
                         three_alt_m %>% mutate(model="3"),
                         four_alt_m %>% mutate(model="4"),
                         five_alt_m %>% mutate(model="5"))
d_res <- d_joined_models %>%
  split(list(.$model, .$domain, .$scale)) %>%
  map_df(~rsa.runDf(data=.x,
                    quantityVarName="stars",
                    semanticsVarName="sems",
                    itemVarName="item"))
d_res[get_compare_indices(d_res, model="2"),] %>%
  filter(domain=="restaurant") %>%
  gather(type, value, c(prags, preds)) %>%
  ggplot(aes(x=as.numeric(stars), y=value, lty=type, col=item)) +
    geom_line() +
    facet_grid(model~scale)

d_res[get_compare_indices(d_res, model="2"),] %>%
  group_by(model) %>%
  summarise(r=cor(preds, prags))
```
...
curves %>%
  ggplot(aes(x=alpha, y=r, col=as.character(model))) +
    geom_line() +
    ggtitle("Correlation curves\ntuned alpha\ndepth=1") +
    theme_bw()
```

With a single recursion (depth=1) and tuning `alpha` we see substantial improvement in model fit with the inclusion of more alts. This is consistent with findings from our original study.

```{r best_r_res, results='asis'}
curves %>%
  group_by(model) %>%
...
Our best correlation in each case is lower than in our first study. We are aggregating over both `domain` and `scale`, however.

```{r agg_over_scale_domain_res}
if (!("m_test1" %in% ls(envir=.GlobalEnv))) {
...
Using our best `alpha` tuned with `depth=1` take a look at our data...
```{r}
...
for (i in 1:nrow(m_test1)) {
  curr_alpha <- m_test1[i, ]$alpha
  curr_depth <- m_test1[i, ]$depth
  curr_model_num <- m_test1[i, ]$model
  
  curr_d <- d_joined_models %>%
    filter(model==curr_model_num) %>%
    split(list(.$domain, .$scale)) %>%
    map_df(~rsa.runDf(data=.x,
                      quantityVarName="stars",
                      semanticsVarName="sems",
  ...
  d_best_models <- rbind(d_best_models, curr_d)
}

d_best_models[get_compare_indices(d_best_models, model="2"),] %>%
  filter(domain=="restaurant") %>%
  gather(type, value, c(prags, preds)) %>%
  ggplot(aes(x=as.numeric(stars), y=value, lty=type, col=item)) +
    geom_line() +
...
d_best_models[get_compare_indices(d_best_models, model="2"),] %>%
  group_by(model, domain, scale) %>%
  summarise(r=cor(preds, prags)) %>%
  arrange(domain, scale, model)
...
```{r exp1b, results='asis'}
m_test1b <- read.csv("/Users/benpeloquin/Desktop/Projects/scalar_alts/analysis/cache/m_test1b.csv")
if (!("m_test1b" %in% ls(envir=.GlobalEnv))) {
  ...
  m_test1b <- rbind(two_alt_res %>% mutate(model="2"),
                    three_alt_res %>% mutate(model="3"),
                    four_alt_res %>% mutate(model="4"),
                    five_alt_res %>% mutate(model="5"))
  write.csv(m_test1b, "/Users/benpeloquin/Desktop/Projects/scalar_alts/analysis/cache/m_test1b.csv")
}
...
m_test1b %>%
  ...
  mutate(diff=best_r-L0_r) %>%
  knitr::kable()
```

This is interesting - aggregating over scale and domain, our 2-, 3- and 4-alt listeners would be more effective (accurate) by avoiding inference and simply considering the literal semantics. Only once we get to 5-alts does RSA provide any benefits at they're not too much.
...
m_test2 %>%
  ...
  summarise(num_runs=n(),
            num_run_recurse=sum(best_depth > 0),
            avg_alhpa=mean(best_alpha),
...
```
Here I'm showing the total number of times that the model needed to recurse at least once with `num_run_recurse` (out of `num_runs`) as well as the average `alpha` level for each model. So out of 30 runs, 26 in the 5-model case required recursion with an average overall alpha level. This supports the (emerging story), it really only makes sense to reason about a speaker if you have alternatives available. Does this start to fit into a developmental story (Rose's study)?
...
-- `good_excellent` and `liked_loved`: 5-alt model pushes pragmatic judgments to 1 and 0, which leads to improved fit over semantics. 

-- `memorable_unforfettable`: high fit with L0 and really only improves at M3.

-- `bad_terrible` is difficult in part because there really isn't an implicature.
...
# d_best_models[compare_indices,] %>%
#   group_by(model, scale, domain) %>%
#   summarise(avg_num_recurse=mean(depth),
#             avg_alpha=mean(alpha),
#             avg_r=cor(preds, prags)) %>%
#   gather(param_type, param_value, c(avg_num_recurse, avg_alpha)) %>%
#   ggplot(aes(x=avg_r, y=param_value, col=as.factor(param_type))) +
...
#             avg_alpha=mean(alpha),
#             avg_r=cor(preds, prags)) %>%
#   gather(param_type, param_value, c(avg_num_recurse, avg_alpha, model)) %>%
#   ggplot(aes(x=param_value, y=avg_r, col=scale, shape=domain)) +
...
d_best_models[compare_indices,] %>%
  ...
  ggplot(aes(x=param_value, y=avg_r, col=scale, shape=domain)) +
    geom_jitter(width = 0.25, alpha=0.6) +
    facet_grid(model~param_type, scales="free") +
    theme_bw()
```

I think this is a visual that gets at the story a little -- we can see the increase impact of `alpha` and `depth` as we include more alternatives. Avg correlation (y-axis) increases while the parameter settings do as well (x-axis). The spreading off the y-axis is indicates that the model is doing work over and above the literal semantics.

```{r best_models_summary2, results='asis'}
d_best_models[compare_indices,] %>%
  group_by(model) %>%
  summarise(avg_num_recurse=mean(depth),
            avg_alpha=mean(alpha),
...
```{r best_models_summary3, results='asis'}
d_best_models[compare_indices,] %>%
...
--- If people are reasoning about one another having a rich set of alternatives is beneficial (not just entailment alts).

-- Model preds are sensitive to differences in `scale` and `domain` (we get best overall performance when we fit at these levels).

-- If you don't have those alternatives available, you might as well stick with the literal semantics (which will get you pretty far).

-- Is this consistent with a developmental account (Rose's paper)?

- i.e. inference requires both the ability to reason about others (theory of mind???).

- as well as acquisition of conventionalized alternatives sets (look at wordbank ages for quantifiers and scalar adjectives)

- how do depth and alpha really trade off? what is depth really getting at?

Do we see this with our cogsci data?
```{r compare_with_cogsci, results='asis'}
peloquinFrank_2Alts %>%
  group_by(scale) %>%
  summarise(r=cor(speaker.p, e11)) %>%
  knitr::kable()
```

We also see (fairly) good fit between literal semantics with cogsci L0 data nd pragmatic judgments.
