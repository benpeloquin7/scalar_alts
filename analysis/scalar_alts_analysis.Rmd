  ...
  if (study_type == "L0") {
    bad_ids <- d %>%
      ...
      filter(training_perf != 2,
             tolower(language) != "english") ## filter out anyone who doesn't pass both
...
d_album <- create_domain_df("album", dir_path=create_dir_path("album", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
d_book <- create_domain_df("book", dir_path=create_dir_path("book", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
d_game <- create_domain_df("game", dir_path=create_dir_path("game", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
d_movie <- create_domain_df("movie", dir_path=create_dir_path("movie", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
d_play <- create_domain_df("play", dir_path=create_dir_path("play", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
d_restaurant <- create_domain_df("restaurant",
                                 dir_path=create_dir_path("restaurant", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)

d_agg <- rbind(d_album, d_book, d_game, d_movie, d_play, d_restaurant)
```
...
d_bored <- d_agg %>%
  filter(item == "boring")
head(d_bored)
summary(glm(judgment~domain+stars, family="binomial", data=d_bored))
...
d_album <- create_domain_df("album", dir_path=create_dir_path("album", "L1"), study_type="L1", filter=TRUE, verbose=TRUE)
# d_book <- create_domain_df("book", dir_path=create_dir_path("book", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
# d_game <- create_domain_df("game", dir_path=create_dir_path("game", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
# d_movie <- create_domain_df("movie", dir_path=create_dir_path("movie", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
# d_play <- create_domain_df("play", dir_path=create_dir_path("play", "L0"), study_type="L0", filter=TRUE, verbose=TRUE)
...
d_album %>% View
  group_by(item, judgment) %>%
  summarise(counts=n()) %>%
  ggplot(aes(x=judgment, y=counts)) +
    geom_bar(position="dodge", stat="identity") +
    facet_wrap(~item)
ggplot(d_album, aes(x=judgment, y=avg_judgment, col=domain)) +
  geom_line() +
  geom_errorbar(aes(ymax=ci_high, ymin=ci_low), width=0) +
  # geom_pointrange(aes(ymax=ci_high, ymin=ci_low)) +
  facet_wrap(~item)
...
d_L0_scales_2alt <- d_L0_scales %>%
  ...
  filter(word_type=="hi1" | word_type=="h2")
  
plyr::ddply(.data=d_L0_scales_2alt, .fun=rsa.runDf, 
            .variables=c("domain", "scale"), quantityVarName="stars",
            semanticsVarName="sems", itemVarName="item", alpha=10) %>%
  filter((word_type=="hi1" | word_type=="hi2"),
         scale=="good_excellent")

d_L0_movies <- d_L0_scales %>%
  filter(domain=="movie")
         # (word_type=="hi1" | word_type=="hi2"))
movie_preds <- plyr::ddply(.data=d_L0_movies, .fun=rsa.runDf, 
                           .variables=c("scale"), quantityVarName="stars",
                           semanticsVarName="sems", itemVarName="item", alpha=3)
movie_preds %>% filter(scale == "memorable_unforgettable")
  
 # plyr::ddply(data, .fun = rsa.runDf, 
 #                  .variables = c(groupName), quantityVarName = quantityVarName, 
 #                  semanticsVarName = semanticsVarName, itemVarName = itemVarName, 
 #                  costsVarName = costsVarName, priorsVarName = priorsVarName, 
