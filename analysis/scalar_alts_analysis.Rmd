...
rbind(best_two_alpha %>% mutate(model="2"),
...
### Experiment 2: Tune depth alpha for each `model` and `scale` aggregating `domain`
```{r}
...
best_2 <- two_alt_res %>%
  ...
  summarise(best_r=max(r),
            ...
            best_depth=depth[which.max(r)])
two_alt_m %>%
  gather(type, value, c(sems, prags)) %>%
...
best_5 <- five_alt_res %>%
  ...
  summarise(best_r=max(r),
            ...
            best_depth=depth[which.max(r)])
```
Here again, it appears that some scales are differentially sensitive to the need for pragmatics and alternatives. 

```{r}
d_best <- rbind(best_2 %>% mutate(model="2"),
      best_3 %>% mutate(model="3"),
      best_4 %>% mutate(model="4"),
      best_5 %>% mutate(model="5"))

ggplot(d_best, aes(x=model, y=best_r)) +
  geom_bar(stat="identity", position="dodge") +
  ylim(0, 1) +
  facet_wrap(~scale)
```
...
best_3 <- three_alt_res %>%
  ...
  summarise(best_r=max(r),
            best_alpha=alpha[which.max(r)],
            best_depth=depth[which.max(r)]) %>% View
mean(best_3$best_r)
```
...
Basic RSA functionality debugging
```{r eval=FALSE}
## BP work 8.19.16
## build off this framework

ge_models <- rbind(five_alt_m %>% mutate(model="5"),
                   four_alt_m %>% mutate(model="4"),
                   three_alt_m %>% mutate(model="3"),
                   two_alt_m %>% mutate(model="2")) %>%
  filter(scale=="memorable_unforgettable")
ge_res <- data.frame(r=c(), model=c(), domain=c(), alpha=c(), depth=c())

ALPHAS <- seq(0, 2, by=0.1)
DEPHS <- seq(0, 2)
MODELS <- as.character(seq(2, 5))
        ...
        for (depth in DEPTHS) {
          out1 <- ge_models %>%
            filter(model==model_num, 
                   domain==domain_type)
          curr_r <- rsa.runDf(out1,
                   quantityVarName="stars",
                   semanticsVarName="sems",
                   itemVarName="item",
                   alpha=alpha,
                   depth=depth) %>%
            filter(item=="memorable" | item=="unforgettable") %>%
            select(preds, prags) %>%
            cor
          
          new_d <- data.frame(r=curr_r[1,2], model=model_num, domain=domain_type, alpha=alpha, depth=depth)
          ge_res <- rbind(ge_res, new_d)
        }
      }
    }  
  }
...
ge_res %>%
  ...
  summarise(max_r=max(r),
            ...
            max_depth=depth[which.max(r)]) %>% View

debug(f)
f()


album5 <- five_alt_m %>% filter(domain=="album")
...
five_alt_m %>%
  ...
  map_df(~rsa.runDf(
    ...
    depth=1,
    alpha=16)) %>%
  filter(scale=="liked_loved", (item=="liked"|item=="loved")) %>%
  # select(preds, prags) %>%
  # cor()
  # 
  gather(type, value, c(preds, prags)) %>%
  ggplot(aes(x=as.numeric(stars), y=value, lty=as.factor(type), col=as.factor(item))) +
  geom_line() +
  facet_wrap(~domain)
